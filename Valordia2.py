...
arquivo_csv = "dados_operacoes1.csv"
VALOR_TAXA_REGISTRO = 0.16
VALOR_TAXA_EMOLUMENTOS = 0.09

robos_disponiveis = ["Termômetro", "Sem XP", "Sem Credit"]
...
def atualizar_tabela():
    ...
    if os.path.exists(arquivo_csv):
        ...
        df_grouped = df.groupby("Data", as_index=False)[
            ["Taxa de Registro (R$)", "Taxa de Emolumentos (R$)", "Valor da Operação (R$)"]
        ].sum()
        ...
        df_grouped["Data"] = df_grouped["Data"].dt.strftime("%d/%m")

        for _, row in df_str.iterrows():
            valores = list(row)
            valores[4] = f"{float(row['Taxa de Registro (R$)']):.2f}".replace(".", ",")
            valores[5] = f"{float(row['Taxa de Emolumentos (R$)']):.2f}".replace(".", ",")
            valores[6] = f"{math.floor(float(row['IR (R$)'])*100)/100:.2f}".replace(".", ",")
            tabela.insert("", tk.END, values=valores)

        for _, row in df_grouped.iterrows():
            tabela.insert("", tk.END, values=[
                f"TOTAL {row['Data']}", "", "", f"{row['Valor da Operação (R$)']:.2f}".replace(".", ","),
...
def excluir_registro():
    ...
    if os.path.exists(arquivo_csv):
        ...
        df = df[~(
            (df["Data"] == valores[0]) &
            (df["Número de Operações"] == valores[1]) &
            (df["Robô"] == valores[2]) &
